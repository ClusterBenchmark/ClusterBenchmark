...
timeout --kill-after=10s $(($TIMEOUT * $K + 1800)) /usr/bin/time -v python3 run_combo.py --input_file "$INPUT_FILE" --output_file "$BASENAME""_combo_" --verbose 0 --k "$K" --timeout "$TIMEOUT" > "$BASENAME"_combo_out.txt 2> "$BASENAME"_combo_time_mem.txt

STATUS=$?

PYTHON_OUT=$(cat "$BASENAME"_combo_out.txt | tail -1)
MAX_MEM=$(cat "$BASENAME"_combo_time_mem.txt | grep 'Maximum resident set size' | awk '{print $6}')

rm "$BASENAME"_combo_out.txt

for i in $(seq 1 $K);
do
    echo -n "combo,$BASENAME,$i,$MAX_MEM,$STATUS"

    FILE="$BASENAME"_combo_$((i - 1)).txt
