...
timeout --kill-after=10s $(($TIMEOUT * $K + 1800)) /usr/bin/time -v python3 run_wt.py --input_file "$INPUT_FILE" --output_file "$BASENAME""_wt_" --verbose 0 --k "$K" --timeout "$TIMEOUT" > "$BASENAME"_wt_out.txt 2> "$BASENAME"_wt_time_mem.txt

STATUS=$?

PYTHON_OUT=$(cat "$BASENAME"_wt_out.txt)
MAX_MEM=$(cat "$BASENAME"_wt_time_mem.txt | grep 'Maximum resident set size' | awk '{print $6}')

rm "$BASENAME"_wt_out.txt

for i in $(seq 1 $K);
do
    echo -n "wt,$BASENAME,$i,$MAX_MEM,$STATUS"

    FILE="$BASENAME"_wt_$((i - 1)).txt
