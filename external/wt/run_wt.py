...
def run_wt(g):
    """
    Run the WT community detection algorithm on the graph.
...
def run_wt_worker(g, result_queue):
    """
    A worker function to run WT in a separate process.
    Puts the result in a queue.
    """
    try:
        membership, modularity = run_wt(g)
...
def main():
    parser = argparse.ArgumentParser(description="Run WT clustering.")
    ...
    if (args.verbose):
        print("Running WT clustering ...")

    for i in range(args.k):
        gc.collect()
        result_queue = multiprocessing.Queue()
        p = multiprocessing.Process(target=run_wt_worker, args=(g, result_queue))
