...
/usr/bin/time -v python3 gnns.py --input "$INPUT_FILE" --output "$BASENAME" --K "$K" --timeout "$TIMEOUT" --num-processes "$THREADS" > "$BASENAME"_gnns_out.txt 2> "$BASENAME"_gnns_time_mem.txt
