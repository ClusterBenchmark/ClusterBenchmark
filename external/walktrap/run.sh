...
/usr/bin/time -v python3 run_walktrap.py --input_file "$INPUT_FILE" --output_file "$BASENAME""_walktrap_" --verbose 0 --k "$K" --timeout "$TIMEOUT" > "$BASENAME"_walktrap_out.txt 2> "$BASENAME"_walktrap_time_mem.txt

PYTHON_OUT=$(cat "$BASENAME"_walktrap_out.txt)
MAX_MEM=$(cat "$BASENAME"_walktrap_time_mem.txt | grep 'Maximum resident set size' | awk '{print $6}')
...
do
    ...
    if [ -f $FILE ]; then
        ...
        if [ -f $LABEL_FILE ]; then
            ...
            echo -n ",$TIME,$EVAL_MOD,$EVAL_N,$EVAL_CC,$EVAL_F1,$EVAL_AC,$EVAL_TP,$EVAL_FP,$EVAL_TN,$EVAL_FN"
        else
