...
timeout --kill-after=10s $(($TIMEOUT * $K + 1800)) /usr/bin/time -v python3 run_cnm.py --input_file "$INPUT_FILE" --output_file "$BASENAME""_cnm_" --verbose 0 --k "$K" --timeout "$TIMEOUT" > "$BASENAME"_cnm_out.txt 2> "$BASENAME"_cnm_time_mem.txt

STATUS=$?

PYTHON_OUT=$(cat "$BASENAME"_cnm_out.txt | tail -1)
MAX_MEM=$(cat "$BASENAME"_cnm_time_mem.txt | grep 'Maximum resident set size' | awk '{print $6}')

rm "$BASENAME"_cnm_out.txt

for i in $(seq 1 $K);
do
    echo -n "cnm,$BASENAME,$i,$MAX_MEM,$STATUS"

    FILE="$BASENAME"_cnm_$((i - 1)).txt
