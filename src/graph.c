...
graph *graph_copy(graph *g)
{
    ...
    g->EW = malloc(sizeof(long long) * g->m);
...
/*
    TODO: Make generic and move to util
*/
...
graph *graph_contract_clusters(graph *g, int nc, int *C)
{
    ...
    int *Count = calloc(gc->n, sizeof(int));
    long long *Volume = calloc(gc->n, sizeof(long long));
    ...
    while (p < g->n)
    {
        ...
        for (long long i = g->V[u]; i < g->V[u + 1]; i++)
        {
            ...
            if (Volume[_v] == 0)
                Count[d++] = _v;
            Volume[_v] += g->EW == NULL ? 1 : g->EW[i];
        }

        p++;

        if (p < g->n && _u == C[Order[p]])
            continue;
