...
\subsubsection{\textbf{Greedy agglomeration and multilevel local search}}
...
\paragraph{Clauset--Newman--Moore (\textsc{CNM})~\cite{cnm2004}}
\textsc{CNM} is a hierarchical agglomerative algorithm that greedily maximizes modularity.
...
\paragraph{Wakita--Tsurumi (\textsc{WT})~\cite{wt2007}}
\textsc{WT} refines \textsc{CNM} by addressing its tendency to produce highly unbalanced merges that degrade scalability on large graphs.
...
\paragraph{\textsc{Louvain}~\cite{louvain2008}}
\textsc{Louvain} replaces community-pair merging with greedy local vertex moves.
...
\paragraph{\textsc{Leiden}~\cite{leiden2019}}
\textsc{Leiden} addresses \textsc{Louvain}'s tendency to produce disconnected communities by introducing a refinement phase before contraction.
...
\subsubsection{\textbf{Non-modularity baselines}}
...
\paragraph{\textsc{WalkTrap}~\cite{walktrap2005}}
\textsc{Walktrap} performs hierarchical clustering based on the intuition that random walks on a graph tend to get "trapped" into densely connected regions corresponding to communities. 
...
\paragraph{\textsc{Infomap}~\cite{infomap2008}}
Unlike prior approaches, \textsc{Infomap} formulates community detection as an information-theoretic problem, seeking a clustering that minimizes the expected description length of a random walk under the map equation.
It uses a greedy multilevel search inspired by \textsc{Louvain}, augmented with additional fine- and coarse-tuning steps to escape local minima, similar to \textsc{Leiden}.

\subsubsection{\textbf{Metaheuristics and ensemble/hybrid search}}
...
\paragraph{\textsc{COMBO}~\cite{combo2014}}
Instead of relocating individual vertices as in \textsc{Louvain}, \textsc{COMBO} considers pairs of communities and evaluates full redistributions of vertices between them using Kernighan--Lin–style refinements.
...
\paragraph{\textsc{VieClus}~\cite{vieclus2018}}
\textsc{VieClus} further extends ensemble-based optimization by adopting a memetic (evolutionary) framework.
...
\subsubsection{\textbf{Streaming algorithms}}
...
\paragraph{\textsc{Hollocou}~\cite{hollocou2017}}
\textsc{Hollocou} is a one-pass edge-streaming algorithm based on the observation that, under random stream orderings, intra-community edges tend to arrive earlier than inter-community edges.
...
\paragraph{\textsc{CluStRE}~\cite{clustre2025}}
...
\subsubsection{\textbf{Exact solvers}}
...
\paragraph{\textsc{Bayan}~\cite{bayan2024}}
\textsc{Bayan} is an exact branch-and-cut algorithm for modularity maximization.
...
\subsubsection{\textbf{Continuous and differentiable modularity maximization}}
...
\paragraph{\textsc{DMoN}~\cite{dmon2023}}
\textsc{DMoN} introduces a differentiable relaxation of modularity and realizes it using a GNN.
...
\paragraph{\textsc{GNNS}~\cite{gnns2022}}
\textsc{GNNS} formulates modularity maximization as a continuous optimization process over soft cluster assignments.
...
\paragraph{\textsc{DGCluster}~\cite{dgcluster2024}}
Most learning-based modularity methods, including \textsc{DMoN}, require the number of communities to be specified a priori (see Table~\ref{tab:comp}). \textsc{DGCluster} eliminates this requirement by learning node embeddings with a GNN and clustering implicitly through a joint optimization objective.
...
\paragraph{\textsc{LIM} (Less Is More)~\cite{lim2025}}
...
\textsc{LIM} addresses this trade-off with a single-objective formulation that still does not require prior specification of the number of communities.
...
\subsubsection{\textbf{Contrastive/infomax objectives}}
...
\paragraph{\textsc{CommDGI} (Community Deep Graph Infomax)~\cite{commdgi2020}}
\textsc{CommDGI} learns community-aware node representations by jointly maximizing mutual information between nodes and the graph and between nodes and their communities.
...
\paragraph{\textsc{UCoDe} (Unified Community Detection)~\cite{ucode2023}}
\textsc{UCoDe} optimizes a single contrastive objective derived from the modularity matrix, encouraging high similarity within communities and low similarity across communities.
...
\paragraph{\textsc{MAGI}~\cite{magi2024}}
