...
Classical modularity-based algorithms operate solely on the graph structure $(V,E)$, while learning-based methods additionally exploit node attributes $X$ when available. 
In particular, learning-based approaches often aim to discover communities that are coherent both in terms of graph connectivity and attribute similarity.
...
Given a graph $G=(V,E,X)$, a GNN updates each vertex representation as a function of its own attributes and the representations of neighboring vertices, thereby encoding local graph structure and, through multiple layers, higher-order connectivity patterns.
