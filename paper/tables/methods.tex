...
\begin{table}
    \caption{Comparison of methods included in the experimental survey. \textit{Code} indicates implementation availability ($\times$: unavailable; $\star$: available but not ready for use; $\star\star$: open source with well-defined I/O). \textit{Features} indicate parallelism (P), automatic or user-specified number of clusters (A/K), learning-based methods (L), use of node features (F), and availability in portable Python packages (Py). The two bold methods are exact algorithms. }
    \label{tab:comp}
    ...
    \begin{tabular}{lrrp{2cm}}
        ...
        \midrule
        \textsc{CNM}~\cite{cnm2004}                & 2004 & $\star\star$ & \texttt{. A . . Py} \\
        \textsc{Walktrap}~\cite{walktrap2005}      & 2005 & $\star\star$ & \texttt{. A . . Py} \\
        \textsc{WT}~\cite{wt2007}                  & 2007 & $\star\star$ & \texttt{. A . . Py} \\
        \textsc{Louvain}~\cite{louvain2008}        & 2008 & $\star\star$ & \texttt{. A . . Py} \\
        \textsc{CGGCI}~\cite{cggci_rg2012}         & 2012 & $\times$     & \texttt{. A . . .}  \\
        \textsc{VNS}~\cite{vns2012}                & 2012 & $\times$     & \texttt{. A . . .}  \\
        \textsc{COMBO}~\cite{combo2014}            & 2014 & $\star\star$ & \texttt{. A . . Py} \\
        \textsc{ADVNDS}~\cite{advnds2019}          & 2017 & $\times$     & \texttt{. A . . .}  \\
        \textsc{Hollocou}~\cite{hollocou2017}      & 2017 & $\star\star$ & \texttt{. A . . .}  \\
        \textsc{VieClus}~\cite{vieclus2018}        & 2018 & $\star\star$ & \texttt{P A . . .}  \\
        \textsc{Leiden}~\cite{leiden2019}          & 2019 & $\star\star$ & \texttt{. A . . Py} \\
        \textsc{DMoN}~\cite{dmon2023}              & 2020 & $\star$      & \texttt{P K L F .}  \\
        \textsc{CommDGI}~\cite{commdgi2020}        & 2020 & $\star$      & \texttt{P K L F .}  \\
        \textsc{GNNS}~\cite{gnns2022}              & 2022 & $\star$      & \texttt{P A L . .}  \\
        \textsc{UCoDe}~\cite{ucode2023}            & 2023 & $\star$      & \texttt{P K L F .}  \\
        \textsc{DGCluster}~\cite{dgcluster2024}    & 2023 & $\star$      & \texttt{P A L F .}  \\
        \textsc{MAGI}~\cite{magi2024}              & 2024 & $\star$      & \texttt{P K L F .}  \\
        \textbf{\textsc{Bayan}}~\cite{bayan2024}   & 2024 & $\star\star$ & \texttt{. A . . .}  \\
        \textsc{CluStRE}~\cite{clustre2025}        & 2025 & $\star\star$ & \texttt{. A . . .}  \\
        \textsc{LIM}~\cite{lim2025}                & 2025 & $\star$      & \texttt{P A L F .}  \\
